...
- June 27, 2025. Added comprehensive DeepSeek R1 integration featuring: complete API client for DeepSeek local server (port 5001), dedicated models and generation pages with modern UI design, React Query hooks for data management, sidebar navigation with dedicated DeepSeek section, support for all DeepSeek API endpoints including health checks, model listing, text generation with configurable parameters, quick testing functionality, comprehensive model selection guide with GGUF format support, technical styling consistent with existing design patterns
- June 27, 2025. Removed all HuggingFace related code: deleted HuggingFace pages (Models, Generation, Stats), removed HuggingFace API client and React hooks, cleaned up navigation and routing, removed ApiKeyConfig component that was specific to HuggingFace API key management
- June 27, 2025. Created comprehensive DeepSeek API Documentation page featuring: quick start guide with setup commands, detailed endpoint documentation for all DeepSeek APIs, parameter descriptions with required and optional fields, temperature guide for generation control, response format examples for success and error cases, code examples in cURL, Python, JavaScript, and Node.js, live API testing interface with model selection, model selection guide with GGUF format recommendations, troubleshooting section for common issues, integrated into sidebar navigation under DeepSeek section
