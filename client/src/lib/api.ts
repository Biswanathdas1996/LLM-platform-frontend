      ...
      setTimeout(() => {
        ...
        for (let day = 0; day < 7; day++) {
          ...
          for (let hour = 0; hour < 24; hour++) {
            ...
            const requestCount = Math.floor(Math.random() * baseRequestCount) + 3;
            
            for (let req = 0; req < requestCount; req++) {
              const requestTime = hourStart - (Math.random() * 60 * 60 * 1000);
              const requestId = `req_${requestTime}_${Math.random().toString(36).substr(2, 9)}`;
              
              // Randomly select endpoint type
              const endpoints = [
                { name: "api.health_check", method: "GET", path: "/api/v1/health", avgTime: 15, errorRate: 0.01 },
                { name: "api.list_models", method: "GET", path: "/api/v1/models", avgTime: 35, errorRate: 0.02 },
                { name: "api.generate", method: "POST", path: "/api/v1/generate", avgTime: 2500, errorRate: 0.05 },
                { name: "api.upload_model", method: "POST", path: "/api/v1/models/upload", avgTime: 15000, errorRate: 0.1 },
                { name: "api.delete_model", method: "DELETE", path: "/api/v1/models/delete", avgTime: 500, errorRate: 0.03 },
                { name: "api.cache_status", method: "GET", path: "/api/v1/cache/status", avgTime: 25, errorRate: 0.01 },
                { name: "api.clear_cache", method: "POST", path: "/api/v1/cache/clear", avgTime: 800, errorRate: 0.02 },
                { name: "api.sync_models", method: "POST", path: "/api/v1/models/sync", avgTime: 1200, errorRate: 0.04 }
              ];
              ...
              const responseTime = endpoint.avgTime + (Math.random() - 0.5) * endpoint.avgTime * 0.4;
              
              // Request log
              const requestLog: ExternalLogEntry = {
                args: endpoint.method === "POST" ? { model: `model_${Math.floor(Math.random() * 5)}` } : {},
                content_length: endpoint.method === "POST" ? Math.floor(Math.random() * 1000) + 100 : null,
                content_type: endpoint.method === "POST" ? "application/json" : null,
                ...
                type: "request",
                url: `http://127.0.0.1:5000${endpoint.path}`,
                user_agent: [
              ...
              if (isError) {
                ...
                const errorLog: ExternalLogEntry = {
                  ...
                  status_code: [400, 401, 403, 404, 429, 500, 502, 503][Math.floor(Math.random() * 8)],
              ...
              if (Math.random() < 0.3) {
                const executionLog: ExternalLogEntry = {
                  ...
                  level: "INFO",
                  message: `Endpoint ${endpoint.name.replace('api.', '')} completed in ${(responseTime / 1000).toFixed(3)}s`,
                  module: "execution_tracker",
                  status: isError ? "error" : "success",
                  timestamp: new Date(requestTime + responseTime - 10).toISOString(),
        ...
        api_logs.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
        error_logs.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
